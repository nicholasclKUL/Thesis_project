...
unsigned long int n_seed = 1;
  ...
  void get_U(Box &U) const {
    ...
    re.seed(n_seed);
    std::uniform_real_distribution<real_t> unif(50, 300);
    U.lowerbound.setConstant(-unif(re));
    U.upperbound.setConstant(+unif(re));
  }
  ...
  void get_x_init(rvec x_init) const {
    std::default_random_engine re;
    re.seed(n_seed);
    std::uniform_real_distribution<real_t> unif(-1, 0.5);
    x_init.setConstant(0.);
    if (x_init.size() == params.nx){
      ...
      for (size_t i = 0; i < params.N; ++i){
        for (size_t j = 0; j < params.nx; ++j){
          if (real_t(j)*params.Δx < params.dx1*params.L){
            ...
            x_init(i*(params.nx+params.nu)+j) = 2. + unif(re);
          }
        }
      }
    }
